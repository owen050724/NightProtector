...
public class TimerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        starthomeBtn = (Button)findViewById(R.id.tohome);

        starthomeBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Toast.makeText(TimerActivity.this, "홈 화면으로 이동합니다.", Toast.LENGTH_SHORT).show();
...
//secondTV.setText("0" + second);
                            changeTv(secondTV, "0" + second);
                        } else {
//secondTV.setText(Integer.toString(second));
                            changeTv(secondTV, Integer.toString(second));
                        }

                        if(minute <= 9){
                            changeTv(minuteTV, "0" + minute);
                        } else {
                            changeTv(minuteTV, Integer.toString(minute));
                        }

                        if(hour <= 9){
                            changeTv(hourTV, "0" + hour);
                        } else {
                            changeTv(hourTV, Integer.toString(hour));
                        }

// 시분초가 다 0이라면 toast를 띄우고 타이머를 종료한다..
                        if(hour == 0 && minute == 0 && second == 0) {
                            timer.cancel();//타이머 종료
                            changeTv(finishTV, "타이머가 종료되었습니다.");
...
//타이머를 실행
    ...
    void changeTv(TextView et, String number) {
